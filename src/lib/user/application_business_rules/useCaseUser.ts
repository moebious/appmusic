...
export async function getList(req: Request, res: Response): Promise<Response | void> {
    try {
...
export async function createUser(req: Request, res: Response) {
        ...
        if (emailValid === true && passwordValid === true){
            const conn = await connect();
            let user = await conn.query('SELECT * FROM user WHERE user.email = ?', [newUser.email])
            user.find(()=>{
...
export async function getUser(req: Request, res: Response) {
    ...
    const conn = await connect();
    const user = await conn.query('SELECT * FROM user WHERE user.user_id = ?', [id]);
        res.json(user[0]);
}
